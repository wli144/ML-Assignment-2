...
def add_resnet_pca(
    ...
    resnet_csv   : path to the training ResNet feature CSV
                   (resnet_features_hf.csv by default).
    test_csv     : path to the test/val ResNet feature CSV
                   (resnet_features_test.csv by default). Used whenever
                   is_train=False and resnet_* columns are not already present
                   in merged_df (i.e. the held-out val rows from the training
                   CSV are already merged; the unlabelled test set is not).
